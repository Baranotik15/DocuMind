...
- **🗣️ Голосовой разговор без рук** — вторая кнопка открывает живой, непрерывный разговор: она сама определяет, когда вы закончили говорить, и отправляет сказанное без нажатия кнопки, ответ появляется в виде текста и одновременно озвучивается голосом примерно в темпе чтения, а заговорить снова — значит перебить ответ прямо посреди фразы. Распознавание речи остаётся локальным (та же модель Vosk, что и у диктовки); озвучка ответов синтезируется через OpenAI TTS API. Опционально и по умолчанию выключено — см. [Настройка распознавания голоса](#-настройка-распознавания-голоса-опционально).
...
| `WS` | `/chat/voice-session` | Голосовой разговор без рук — непрерывная речь на входе (Vosk), потоковый текст + озвученные ответы на выходе (OpenAI TTS), с возможностью перебить |
...
На странице Chat есть две голосовые фичи, обе построены на одной и той же
локально размещённой модели [Vosk](https://alphacephei.com/vosk/) для
распознавания того, что вы говорите — само распознавание всегда работает
офлайн, аудио вашего голоса никогда не покидает ваш деплой:

- **Кнопка микрофона** позволяет надиктовать вопрос вместо печати: запись
  → распознанный текст в поле ввода → вы сами проверяете/правите и
  отправляете.
- **Кнопка с волной** рядом с ней открывает голосовой разговор без рук:
  она непрерывно слушает, сама отправляет сказанное как только вы
  замолчали (без нажатия кнопки), ответ приходит потоковым текстом и
  озвучивается голосом — заговорить снова посреди ответа сразу его
  перебивает. В отличие от распознавания вашего голоса, **аудио озвучки
  ответа синтезируется через облачный OpenAI TTS API** (используется тот
  же `OPENAI_API_KEY`, что уже нужен чату) — в эту сторону данные
  действительно уходят из вашего деплоя. Если синтез не удался (например,
  ключ не настроен), ответ всё равно приходит текстом, просто без
  озвучки.

Обе фичи опциональны и по умолчанию выключены: без настроенной модели Vosk
кнопка микрофона всё ещё записывает звук, но распознавание завершается
понятной ошибкой "модель не установлена", а сессия голосового разговора
точно так же завершается с ошибкой вместо того чтобы молча ничего не
делать.
...
**Озвучка ответов в голосовом разговоре** не требует отдельной настройки
сверх `OPENAI_API_KEY`, который чату и так нужен. Модель и голос TTS
настраиваются через опциональные переменные в `.env` (см. `.env.example`):
...
Один голос на весь деплой — выбора по пользователю или по сообщению нет.
